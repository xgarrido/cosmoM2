...
for exp in ["LAT", "Planck"]:
    ...
    for cross in cwr(my_freqs, 2):
        ...
        if (len(np.nonzero(n_ell_t[name])[0])) != 0:  # plot only the spectra that are non zero
            ...
            if not(os.path.isdir('sim_data/noise_tot_test/')):
                os.mkdir('sim_data/noise_tot_test/')
            np.savetxt("sim_data/noise_tot_test/noise_t_%s.dat"%(name), np.transpose([ell,n_ell_t[name] * fac / (bl[exp + f1] * bl[exp + f2])]))
            ...
            plt.plot(ell, n_ell_pol[name] * fac / (bl[exp + f1] * bl[exp + f2]),
                     linestyle=linestyle[exp],
                     label="%s" % (name))
            np.savetxt("sim_data/noise_tot_test/noise_pol_%s.dat"%(name), np.transpose([ell,n_ell_pol[name] * fac / (bl[exp + f1] * bl[exp + f2])]))
            plt.xlabel(r"$\ell$", fontsize=22)
